...
export const OrdersTable: React.FC = function OrdersTable() {
  ...
  var { orders, UpdateOrder, currentUser, userRolef, refreshingOrderTables } =
    value;
  ...
  const handleSubmit = async (status, orderIndex) => {
    try {
      ...
      let order = {
        ...orders[orderIndex],
        OrderStatus: status,
        Rider: orders[orderIndex].Rider._id,
      };
      //console.log(order);
    ...
    if (userRolef === "Admin" || userRolef === "Urged_Staff") {
      orders.map((item, index) => {
        ...
        orderItems = item.OrderItems.map((item, index) => {
          ...
          return (
            <>
              <Typography style={{ fontWeight: 900 }}>{IName}</Typography>
              <Typography>{IFlavor1}</Typography>
              <Typography>{IFlavor2}</Typography>
              <Typography>{Drink}</Typography>
              <Typography>{fianlDecision}</Typography>
            </>
          );
        });
        let row = {
          ...
          Actions: (
            <>
              <a
                href="/AdminOrderSDetails"
                title="edit"
                onClick={(e) => {
                  e.preventDefault();
                  history.push("/AdminOrderSDetails", { from: index });
                }}
              >
          ...
          <div className={clsx(classes.paper, "modalMobile")}>
            ...
            >
              <Grid item xs={12} style={{ textAlign: "center" }}>
                <Button
                  variant="contained"
                  style={{ backgroundColor: "red", fontFamily: "PT Sans" }}
