...
export const OrgDetails: React.FC = function OrgDetails() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      if (
        values.Menu.length > rows.length ||
        rows.length === 0 ||
        (values.Menu.length < rows.length && values.Menu.length !== 0)
      ) {
  ...
  const handleChange4 =
    (prop: keyof MenuItem) => (event: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleOpen = () => {
    try {
      setSelectedMenuItem({
        ...
        ItemCost: 0,
        ItemDescription: "",
        ImageName: "Images/FoodItemPH.png",
      });
      setOpen(true);
    } catch (err) {}
  };
