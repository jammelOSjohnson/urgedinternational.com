...
export const CheckGps: React.FC<Props> = function CheckGps({ setLoading }) {
  ...
  const [gpsCheck, setgpsCheck] = React.useState<NoGps>({
    ...
    open2: false,
    open: false,
    errorMessage: `Please Click ${(
      <a href={history.location.pathname} onClick={turnOnGps}></a>
    )} To Turn On GPS.`,
  });
  ...
  var checkFence = async function checkFence(polygonCoords, lat, lng) {
    if (!gpsCheck.open3) {
      ...
      if (contains) {
        ...
        //console.log(values);
        await UpdateUserInfo(value, values)
                  ...
                  >
                    <Grid item xs={12}>
                      <Typography variant={"caption"}>
                        Please enter your delivery address to verify that we
                    ...
                    <Grid item xs={12}>
                      <TextField
                        id="outlined-multiline-static1"
                        label="Enter Contant Number"
                        // multiline
                        // rows={4}
                        value={values.ContactNumber}
                        onChange={handleChange4("ContactNumber")}
                        variant="outlined"
                        placeholder="Eg. 876-123-1234"
                        helperText="Eg. 876-123-1234"
                        fullWidth
                      />
                    </Grid>
