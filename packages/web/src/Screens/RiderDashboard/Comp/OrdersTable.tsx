...
export const OrdersTable: React.FC = function OrdersTable() {
  ...
  const handleSubmit = async (status, orderIndex) => {
    try {
      ...
      let order = {
        ...
        OrderStatus: status,
        Rider: filteredOrders[orderIndex].Rider._id,
      };
      //console.log(order);
  ...
  const handleReject = async (status, orderIndex) => {
    try {
      ...
      let order = {
        ...filteredOrders[orderIndex],
        Rider: filteredOrders[orderIndex].Rider,
      };
      //console.log(order);
      //console.log(riders);
      ...
      await OrderRejectionList(order).then(async (res) => {
        if (res === null) {
          let filteredRiders = riders.filter(
            (rider) =>
          ...
          Actions: (
            <>
              <a
                ...
                onClick={(e) => {
                  e.preventDefault();
                  history.push("/DeliveryOrdersDetails", { from: index });
                }}
              >
