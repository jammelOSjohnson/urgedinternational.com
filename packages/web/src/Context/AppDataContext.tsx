...
export default function AppDataProvider({ children }: { children: ReactNode }) {
  ...
  const [getRidersByParish] = useMutation(GET_RIDERS_BY_PARISH);
  const [getStaff] = useMutation(GET_STAFF);
  const [getRider] = useMutation(GET_RIDER);
  //const {data} = useQuery(GET_PAY_SETTINGS);
  const [fetchPaySettings] = useMutation(FETCH_PAY_SETTINGS);
  ...
  const [createHash] = useMutation(CREATE_HASH_MUTATION);
  ...
  //CreatHash
  var createPaymentHash = async function createPaymentHash(paymentReq) {
    let result = await createHash({
      variables: {
        ...
        txndatetime: paymentReq.txndatetime,
        txntype: paymentReq.txntype,
      },
    }).then(function (response) {
      return response.data.createHash;
    });
  ...
  var signup2 = function signup2(currentstate, payload) {
    ...
    var result = creatRestauranteUser({ email, password })
  ...
  var userHasRole = async function userHasRole(uid, payload, userType) {
    ...
    let result = await getUserInRole({ variables: { UserID: uid } }).then(
    ...
    if (result !== null) {
      ...
      if (userRoleID !== null) {
        let afterGetRole = await getRole({
          variables: { _id: userRoleID },
        ...
        //console.log("ELSE about to return role result");
        return payload;
      }
    } else {
      ...
      let afterAddUserToRole = await addUserToRole({
        variables: { UserID: userInRole.UserID, RoleID: userInRole.RoleID },
      })
        .then(function (response3) {
          if (response3.data.addUserToRole !== null) {
            // console.log("User successfully added to role!");
            // console.log(userType);
            payload.userRolef = userType === undefined ? "Customer" : userType;
  ...
  var fetchUserInfoForSignUp = async function fetchUserInfoForSignUp(
    ...
    await getUser({ variables: { Id: uid } })
          ...
          var user2 = {
            ...
            LastName:
              payload.userInfo.fullName !== null
                ...
                : "",
            Email:
              payload.userInfo.email !== null ? payload.userInfo.email : "",
            AddressLine1: "",
            AddressLine2: "",
            City: "",
            ContactNumber: "",
          ...
          createUser({ variables: { ...user2 } })
            .then(async function (response2) {
              if (response2.data.createUser !== null) {
                ...
                payload.loggedIn = true;
                payload.userInfo.addressLine1 =
                  user2.AddressLine1 !== null &&
                  user2.AddressLine1 !== undefined
                    ? user2.AddressLine1
                    : "";
                payload.userInfo.addressLine2 =
                  user2.AddressLine2 !== null &&
                  user2.AddressLine2 !== undefined
                    ? user2.AddressLine2
                    : "";
                payload.userInfo.city =
                  user2.City !== null && user2.City !== undefined
                    ? user2.City
                    : "";
  ...
  var restaurantSignUp = async function restaurantSignUp(
    uid,
    payload,
    currentState,
    newRestaurant,
      ...
      .then(function (response) {
        ...
        if (response.data.getUser !== null) {
          ...
          if (user !== null) {
            ...
            payload.userInfo.city =
              user.City !== null && user.City !== undefined ? user.City : "";
            payload.userInfo.deliveryFee =
              user.deliveryFee !== null && user.deliveryFee !== undefined
                ? user.deliveryFee
                : 0;
            payload.userInfo.Parish =
              user.Parish !== null && user.Parish !== undefined
                ? user.Parish
                : "Select Parish";
          }
          ...
          var user2 = {
            ...
            deliveryFee: parseFloat(newRestaurant.deliveryFee),
          };
          //verified: false,
          //verifiedemailsent: false,
          //postalCode: "",
          ...
          createRestaurant({ variables: { ...user2 } })
            .then(async function (response2) {
              if (response2.data.createUser !== null) {
                ////console.log("User info  successfully written!");
                ////console.log(response2.data);
  ...
  var staffSignUp = async function staffSignUp(
    uid,
    payload,
    currentState,
    newStaff,
          ...
          if (user !== null) {
            ...
            payload.userInfo.addressLine2 =
              user.AddressLine2 !== null && user.AddressLine2 !== undefined
                ? user.AddressLine2
                : "";
            payload.userInfo.disabled =
              user.disabled !== null && user.disabled !== undefined
                ? user.disabled
                : "";
            payload.userInfo.isAvailable =
              user.isAvailable !== null && user.isAvailable !== undefined
                ? user.isAvailable
                : "";
            payload.userInfo.city =
              user.City !== null && user.City !== undefined ? user.City : "";
            payload.userInfo.Position =
  ...
  var fetchUserInfo = async function fetchUserInfo(uid, payloadf) {
    ...
    let userFetch = await getUser({ variables: { Id: uid } })
      .then(async function (response) {
    ...
    if (userFetch !== null) {
      ...
      payloadf.userInfo.contactNumber =
        userFetch.ContactNumber !== null &&
        userFetch.ContactNumber !== undefined
          ? userFetch.ContactNumber
          : "";
      payloadf.userInfo.email =
        userFetch.Email !== null && userFetch.Email !== undefined
          ? userFetch.Email
          : "";
      payloadf.userInfo.fullName =
        userFetch.FirstName !== null && userFetch.FirstName !== undefined
          ? userFetch.FirstName
          : "";
      payloadf.userInfo._id =
        userFetch._id !== null &&
        userFetch._id !== undefined &&
        userFetch._id !== ""
          ? userFetch._id
          : "";
      payloadf.loggedIn = true;
      payloadf.userInfo.addressLine1 =
        userFetch.AddressLine1 !== null && userFetch.AddressLine1 !== undefined
          ? userFetch.AddressLine1
          : "";
      payloadf.userInfo.addressLine2 =
        userFetch.AddressLine2 !== null && userFetch.AddressLine2 !== undefined
          ? userFetch.AddressLine2
          : "";
      payloadf.userInfo.city =
        userFetch.City !== null && userFetch.City !== undefined
          ? userFetch.City
          : "";

      //var userRoleResf = undefined;
      afterUserHasRole = await userHasRole(uid, payloadf, undefined).then(
        function (userRoleRes) {
      ...
      var user2 = {
        ...
        LastName:
          payloadf.userInfo.fullName !== null
            ...
            : "",
        Email:
          payloadf.currentUser.email !== null ? payloadf.currentUser.email : "",
        AddressLine1: "",
        AddressLine2: "",
        City: "",
        ContactNumber: "",
      };
      let afterCreateUser = await createUser({ variables: { ...user2 } })
        .then(async function (response2) {
          if (response2.data.createUser !== null) {
            // console.log("User info  successfully written!");
            // console.log(response2.data);
            ...
            payloadf.userInfo._id =
              getUserResult._id !== null &&
              getUserResult._id !== undefined &&
              getUserResult._id !== ""
                ? getUserResult._id
                : "";
          }
      ...
      let afterUserHasRole = await userHasRole(uid, payloadf, undefined)
        .then(async function (userRoleRes) {
          // console.log("Final user ref is: ");
          // console.log(userRoleRes);
          return userRoleRes;
        })
      ...
      let afterSendMail = await sendEmail(
        emailServiceId,
        emailNewCustomerTemplate,
        RequestParams,
        emailUserId,
  ...
  var viewMenuItems = async function viewMenuItems(payload) {
    ...
    if (
      ...
      payload.selectedRestaurantName !== undefined
    ) {
      dispatch({
  ...
  var UpdateOrder = async function UpdateOrder(payload, order) {
    if (order !== null && order !== undefined) {
      ...
      var updateRes = await updateOrder({ variables: newOrder }).then(
        async function (response) {
          ...
          if (response.data.updateOrder !== null) {
            ...
            if (
              newOrder.OrderStatus === response.data.updateOrder.OrderStatus
            ) {
              //console.log("same")
              if (
                newOrder.OrderStatus !== "Not Assigned" &&
  ...
  var CreateOrderRejectionList = async function CreateOrderRejectionList(
    ...
    if (order._id !== undefined) {
      ...
      return await createOrderRejection({
        variables: { OrderId, RejectionList },
      }).then(async function (response) {
  ...
  var UpdateOrderRejectionList = async function UpdateOrderRejectionList(
    ...
    if (order._id !== undefined) {
      ...
      return await updateOrderRejection({
        variables: { _id: _id, OrderId, RejectionList },
      }).then(async function (response) {
    ...
    if (payload.currentUser !== undefined && payload.currentUser !== null) {
      if (payload.currentUser.uid !== undefined) {
        await getOrdersByUserId({
          variables: { Id: payload.currentUser.uid },
        }).then(async function (response) {
  ...
  var fetchOrdersForRider = async function fetchOrdersForRider(payload) {
    if (payload.currentUser !== undefined) {
      ...
      if (payload.userInfo._id !== undefined) {
        await getOrdersByRiderId({
          variables: { Rider: payload.userInfo._id },
        }).then(async function (response) {
  ...
  var fetchOrdersForRider2 = async function fetchOrdersForRider2(
    payload,
    RiderId,
    StartDate,
    EndDate,
    ...
    if (payload.currentUser !== undefined) {
      ...
      if (RiderId !== undefined) {
        await getOrdersByRiderIdAnDate({
          variables: { Rider: RiderId, StartDate, EndDate },
        }).then(async function (response) {
  ...
  var buildTableItem = function buildTableItem(length: number, cartItems) {
    if (length <= cartItems.length) {
      let result =
        ...
        cartItems[length - 1].chickenFlavour1 !== "Select Flavour" &&
        cartItems[length - 1].chickenFlavour1 !== null &&
        cartItems[length - 1].chickenFlavour1 !== undefined
  ...
  var sendOrderCompletedEmail = async function sendOrderCompletedEmail(
    fileType,
    file,
    UserInfo,
    orderNum,
    ...
    if (UserInfo !== null && UserInfo !== undefined && UserInfo.email !== "") {
      ...
      ////console.log(RequestParams)
      var emailRes = await uploadInvoiceEmail(RequestParams, fileType)
  ...
  var sendContactUsEmail = async function sendContactUsEmail(
    user_name,
    user_email,
    subject,
    message,
  ...
  var sendMerchantFormEmail = async function sendMerchantFormEmail(
    fullname,
    businessemail,
    businessname,
    contact,
  ...
  var fetchRestaurantInfo = async function fetchRestaurantInfo(payload, id) {
    ////console.log("about to fetch restaurants");
    await getRestaurant({ variables: { _id: id } })
      .then(async function (response) {
  ...
  var fetchRiderInfo = async function fetchRiderInfo(payload, id) {
    ////console.log("about to fetch restaurants");
    await getRider({ variables: { _id: id } })
      .then(async function (response) {
  ...
  var getPaySettingsData = async function getPaySettingsData(payload) {
    ...
    try {
      ...
      await fetchPaySettings()
        .then((response) => {
          if (response.data.fetchPaySettings !== null) {
            //console.log("got list of restaurants");
            //console.log(response);

            var paySet = response.data.fetchPaySettings;
            //console.log(paySet);

            if (paySet !== null) {
              payload.paySettings =
                paySet !== undefined ? paySet[0] : undefined;
              return payload;
            }
          }
        })
  ...
  var UpdatePaySettings = async function UpdatePaySettings(
    ...
    if (paysetting !== null && paysetting !== undefined) {
      ...
      var updateRes = await updatePaySetting({ variables: newPaySetting }).then(
        async function (response) {
          ////console.log("create orer result");
          ...
          if (response.data.fetchShippingAddress !== null) {
            ...
            if (shippingAdd !== null) {
              payload.shippingAddress =
                shippingAdd !== undefined ? shippingAdd[0] : undefined;
              return payload;
            }
          }
        })
  ...
  var createPreAlert = async function createPreAlert(
    ...
    var RequestParams = {
      ...
      merchant:
        packageZip.merchant !== null && packageZip.merchant !== undefined
          ...
          : "",
      status:
        PackageInfo.Status !== null && PackageInfo.Status !== undefined
          ? PackageInfo.Status
          : "",
      content: file,
    ...
    var checkIfPackageExists = await getPackageById({
      variables: { TrackingNumber: packageZip.trackingNum2 },
    })
      .then(async function (response) {
        console.log("Checking user result for fetch user info");
        //console.log(response.data.getPackageById);
        if (
          response.data.getPackageById !== null &&
          response.data.getPackageById !== undefined
        ) {
          return "Tracking number exist";
        } else {
          console.log("Package with Tracking number does not exist.");
          var storeRes = await addPackage({
            variables: {
              PackageInfo,
              Customer,
              TrackingNumber,
  ...
  var sendPreAlertEmail = async function sendPreAlertEmail(formVals, filetype) {
    //console.log("Wtf is in formVals");
    //console.log(formVals);
    let endingMessage = formVals.Pickup
  ...
  var fetchMailBoxNumberByUserId = async function fetchMailBoxNumberByUserId(
    ...
    //var mbFound = "";
    var mboxRes = await getMailboxById({ variables: { Uid: uid } })
      .then(function (response) {
        //console.log("Logging MailBox");
        if (
          response.data.getMailboxById !== null &&
          response.data.getMailboxById !== undefined
        ) {
  ...
  var mailBoxExist = async function mailBoxExist(number) {
    let finalRes = await getMailboxByMbox({ variables: { MailboxNum: number } })
      .then(function (response) {
        if (
          response.data.getMailboxByMbox !== null &&
          response.data.getMailboxByMbox !== undefined
        ) {
  ...
  var storeMailBoxNumber = async function storeMailBoxNumber(number, uid) {
    ...
    if (!res) {
      var storeRes = await addMailbox({
        variables: { Status: "O", Uid: uid, MailboxNum: number },
      })
        .then(function (response) {
  ...
  var generate = async function generate(uid) {
    ...
    try {
      ...
      var storedNumber = await storeMailBoxNumber(number.toString(), uid)
        .then(function (res) {
  ...
  var UpdateRestaurantBy_ID = async function UpdateRestaurantBy_ID(
    payload: any,
    restaurant:
      | {
          _id: any;
    ...
    if (restaurant !== null && restaurant !== undefined) {
      ...
      var updateRes = await updateRestaurantById({
        variables: {
          _id: restaurant._id,
          Id: restaurant.Id,
